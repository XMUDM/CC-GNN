...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        self.user_pop = self.config['USER_POP']
        self.item_pop = self.config['ITEM_POP']
        self.conf = self.config['CONF']
        self.cold = self.config['COLD']
        self.warm = self.config['WARM']
    ...
    def load_inter_graph(self, file_name):
        ...
        cols = [self.uid_field, self.iid_field, self.splitting_label, self.user_pop, self.item_pop, self.conf, self.cold, self.warm]
