...
class TrainDataLoader(AbstractDataLoader):
    ...
    def _get_neg_sample(self):
        ...
        user_pop_tensor = torch.tensor(cur_data[self.config['USER_POP']].values).type(torch.LongTensor).to(self.device)
        item_pop_tensor = torch.tensor(cur_data[self.config['ITEM_POP']].values).type(torch.LongTensor).to(self.device)
        conf_tensor = torch.tensor(cur_data[self.config['CONF']].values).type(torch.LongTensor).to(self.device)
        ...
        batch_tensor = torch.cat(
                                    (
                                        ...
                                        torch.unsqueeze(user_pop_tensor, 0),
                                        torch.unsqueeze(item_pop_tensor, 0),
                                        torch.unsqueeze(conf_tensor, 0),
                                        torch.unsqueeze(cold_tensor, 0),
                                        torch.unsqueeze(warm_tensor, 0)
                                    )
                                )
