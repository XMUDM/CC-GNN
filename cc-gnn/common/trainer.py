...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        total_mf_loss = None
        total_cl_loss = None
        total_count_loss = None
        loss_batches = []
        for batch_idx, interaction in enumerate(train_data):
            self.optimizer.zero_grad()
            losses, mf_losses, cl_losses, count_losses = loss_func(interaction)
            ...
            if isinstance(losses, tuple):
                ...
                total_mf_loss = mf_losses.item() if total_mf_loss is None else total_mf_loss + mf_losses.item()
                total_cl_loss = cl_losses.item() if total_cl_loss is None else total_cl_loss + cl_losses.item()
                total_count_loss = count_losses.item() if total_count_loss is None else total_count_loss + count_losses.item()
            ...
            #if batch_idx == 0:
            #    break
        self.logger.info('total_loss: {}, total_mf_loss: {}, total_cl_loss:{}, total_count_loss:{}.'.format(total_loss, total_mf_loss, total_cl_loss, total_count_loss))
