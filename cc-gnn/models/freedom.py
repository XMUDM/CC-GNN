...
class FREEDOM(GeneralRecommender):
    ...
    def calculate_loss(self, interaction):
        ...
        pos_e = ia_embeddings[pos_items]
        cold_e = ia_embeddings[cold]
        warm_e = ia_embeddings[warm]
        ...
        maxi = F.logsigmoid(pos_scores - neg_scores)
        batch_mf_loss = -torch.mean(maxi *conf)
        ...
        i_idx = torch.unique(pos_items, return_inverse=True, sorted=False)
        ...
        user_u_e = ua_embeddings[u_id, :]
        item_u_e = ia_embeddings[i_id, :]
        ...
        return batch_mf_loss + self.reg_weight * (mf_t_loss + mf_v_loss) + conter_loss * 0.01 + self.cl_weight * cl_loss, batch_mf_loss + self.reg_weight * (mf_t_loss + mf_v_loss), cl_loss, conter_loss
