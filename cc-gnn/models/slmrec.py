...
class SLMRec(GeneralRecommender):
    ...
    def calculate_loss(self, interaction):
        # multi-task loss
        users, pos, conf = interaction[0], interaction[1], interaction[4]
        cold, warm = interaction[5], interaction[6]
        user_pop = interaction[2]
        item_pop = interaction[3]

        main_loss = self.infonce(users, pos, conf)
        ssl_loss = self.compute_ssl(users, pos)
        cl_loss = self.cl_loss(users, pos, user_pop, item_pop)

        (users_emb, pos_e, neg_emb,
         userEmb0, posEmb0, negEmb0) = self.getEmbedding(users.long(), pos.long(), None)

        (users_emb, warm_e, neg_emb,
        userEmb0, posEmb0, negEmb0) = self.getEmbedding(users.long(), warm.long(), None)
        
        (users_emb, cold_e, neg_emb,
        userEmb0, posEmb0, negEmb0) = self.getEmbedding(users.long(), cold.long(), None)
        ...
        conter_loss1 = -torch.log(pos_score / (pos_score+ttl_score) +10e-6)

        conter_loss = torch.mean(conter_loss1)

        return main_loss + self.config['ssl_alpha'] * ssl_loss + 0.1 * conter_loss + self.config['cl_weight'] * cl_loss, main_loss + self.config['ssl_alpha'] * ssl_loss, cl_loss, conter_loss
    ...
    def infonce(self, users, pos, conf):
    ...
    def cl_loss(self, users, items, user_pop, item_pop):
        ...
        (user_u_e, item_u_e, neg_emb,
            userEmb0, posEmb0, negEmb0) = self.getEmbedding(u_id, i_id, None)
